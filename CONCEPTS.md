...
### Two-pass scrape
The GoComics-only daily schedule that fetches the Reactive favorites page twice — Pass 1 (~03:20 PT) for overnight-syndicated strips and Pass 2 (~13:00 PT) for the mid-morning-Eastern wave — merging both into the same day's JSON. Introduced for #138. The other sources publish reliably enough on a single pass.
